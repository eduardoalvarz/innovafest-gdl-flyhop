...
ApplicationWindow {
    ...
    Component.onCompleted: {
        // Hold the first run prompt(s) back until the splash has cleared, otherwise the first
        // dialog opens behind it and the user just sees a frozen splash.
    ...
    Popup {
        ...
        background: Rectangle {
            ...
            radius:         ScreenTools.panelBorderRadius
    ...
    Popup {
        ...
        background: Item {
            Rectangle {
                ...
                color:          QGroundControl.globalPalette.window
                radius:         ScreenTools.panelBorderRadius
                opacity:        ScreenTools.panelOpacity
                border.width:   1
                border.color:   Qt.rgba(QGroundControl.globalPalette.text.r, QGroundControl.globalPalette.text.g, QGroundControl.globalPalette.text.b, 0.15)
            }
    ...
    //-------------------------------------------------------------------------
    //-- Branded startup screen. Declared last so it stacks above the views.

    OtechSplashScreen {
        id: splashScreen

        onFinished: firstRunPromptManager.nextPrompt()
    }
}
