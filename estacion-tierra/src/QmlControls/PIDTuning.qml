...
RowLayout {
    ...
    // Adopt the size the caller advertises. These are expressed as Layout hints rather than as
    // width/height because our PX4 callers place us inside a ColumnLayout, which owns its
    // children's geometry and would simply overwrite a direct width/height assignment. Callers
    // that are not layouts (the APM page anchors us inside a Loader) ignore these and drive our
    // size through anchors instead. Either way the chart below needs a concrete height or it
    ...
    ColumnLayout {
        ...
        ChartView {
            id:                     chart
            // Let the layout hand us whatever is left over after the right hand panel and the
            // controls below. Computing this by hand from availableWidth/rightPanel.width creates a
            // binding loop (our width feeds the layout which feeds rightPanel.width), which used to
            // resolve to a stale size and clip the plot.
            Layout.fillWidth:       true
            Layout.fillHeight:      true
            Layout.minimumWidth:    _minChartWidth
            Layout.minimumHeight:   _minChartHeight
            antialiasing:           true
