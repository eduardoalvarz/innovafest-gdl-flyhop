...
Item {
    ...
    /// Height the MAVLink console takes along the bottom edge. Published separately from
    /// totalToolInsets because the PiP lives outside this layer, so it is never handed
    /// the inset bundle and has no other way to know the console is down there.
    property real   consoleInset:           _consoleInset
    ...
    // Roomy enough that the rounded corners of the floating panels actually read as floating
    // instead of being jammed into the window edges.
    property real   _toolsMargin:           ScreenTools.defaultFontPixelWidth * 1.6
    ...
    QGCToolInsets {
        ...
        bottomEdgeLeftInset:    Math.max(_consoleInset,
                                    virtualJoystickMultiTouch.visible ? virtualJoystickMultiTouch.bottomEdgeLeftInset : parentToolInsets.bottomEdgeLeftInset)
    ...
    FlyViewBottomRightRowLayout {
        ...
        anchors.margins:    _layoutMargin
        anchors.bottom:     mavlinkConsole.visible ? mavlinkConsole.top : parent.bottom
        anchors.right:      parent.right
        spacing:            _layoutSpacing

        property real bottomEdgeRightInset:     height + _layoutMargin + _consoleInset
        property real bottomEdgeCenterInset:    bottomEdgeRightInset
        property real rightEdgeBottomInset:     width + _layoutMargin
    }

    /// Sits flush along the bottom edge, so everything anchored there has to account for it.
    property real _consoleInset: mavlinkConsole.visible ? mavlinkConsole.height : 0

    FlyViewMavlinkConsole {
        id:             mavlinkConsole
        anchors.left:   parent.left
        anchors.right:  parent.right
        anchors.bottom: parent.bottom
        z:              QGroundControl.zOrderWidgets
        // Yields the screen entirely to a full screen video feed.
        visible:        QGroundControl.settingsManager.flyViewSettings.showMavlinkConsole.rawValue &&
