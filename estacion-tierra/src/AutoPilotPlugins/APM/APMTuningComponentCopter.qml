    ...
    Component {
        ...
        Column {
            ...
            Loader {
                ...
                anchors.left:       parent.left
                anchors.right:      parent.right
                // Claim the rest of the page. Previously this had no height of its own and the
                // PIDTuning inside asked for `availableHeight`, which resolved to its own
                // (zero-valued) property rather than the page's, so the plot collapsed away.
                height:             Math.max(0, tuningColumn.height - y)
